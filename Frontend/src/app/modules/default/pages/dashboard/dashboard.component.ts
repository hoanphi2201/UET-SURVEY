...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(
      this.authService.getCurrentUser().subscribe(userData => {
        ...
        if (this.currentUser && !this.currentUser.accountComplete) {
          ...
          this.subscriptions.push(
            sourceInterval
              .pipe(startWith(0), takeUntil(this.destroyInterval$))
  ...
  private updateIfCompleteAccount() {
    if (
      ...
      this.dUserService.updateUser(updateData, this.currentUser.id).subscribe(
        res => {
          if (res.status.code === 200) {
            this.authService.setCurrentUser(
              { ...this.currentUser, updateData },
              true
            );
          }
        },
  ...
  countSurveyFormStatus() {
    ...
    this.dSurveyFormService.countSurveyFormStatus().subscribe(
      res => {
        if (res.status.code === 200) {
  ...
  get percentDoneProfile() {
    ...
    const totalProfileQuality = Math.floor(
      (<number>fields.reduce((result, next: any) => {
