...
export class CollectEmailComposeComponent implements OnInit {
  @ViewChild("tplContentRecipientsRemoved", { static: false })
  tplContentRecipientsRemoved: TemplateRef<any>;
  ...
  private onAddSurveySecipient(formData: FormGroup) {
    ...
    this.dSurveyRecipientService.addMultySurveyRecipient(recipients).subscribe(
      res => {
        if (res.status.code === 200) {
          if (res.results.every(rec => rec === null)) {
            this.modalService.info({
              nzTitle: this.translateService.instant(
                "default.layout.EMAIL_RECIPIENTS_REMOVED"
              ),
  ...
  onSendMail(formData: FormGroup) {
    ...
    this.dSurveyRecipientService.sendMailSurveyRecipient(recipients).subscribe(
      res => {
        if (res.status.code === 200) {
          this.nzMessageService.success(
            this.translateService.instant(res.status.message)
