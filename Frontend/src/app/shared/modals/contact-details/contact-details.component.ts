...
import { SurveyRecipient, IValidators, DSurveyRecipientService } from '@app/core';
...
export class ContactDetailsComponent implements OnInit {
  ...
  onRemoveRecipient() {
    this.loaderService.display(true);
    this.dSurveyRecipientService.deleteSurveyRecipient(this.surveyRecipientDetails.id).subscribe(res => {
      if (res.status.code === 200) {
        this.nzMessageService.success(this.translateService.instant(res.status.message));
        this.modalService.closeAll();
      }
