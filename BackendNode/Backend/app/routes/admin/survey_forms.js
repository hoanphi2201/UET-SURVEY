...
router.get("/search", async (req, res, next) => {
  ...
  await surveyFormsModel
    .searchSurveyFormsByTitle(title, { limit })
    .then(surveyForms => {
      res
        .status(200)
...
router.get("/surveyFormName/:surveyFormName", async (req, res, next) => {
  const surveyFormName = paramsHelper.getParam(
    req.params,
    "surveyFormName",
    ""
  );
  await surveyFormsModel
    .getSurveyFormsBySurveyFormname(surveyFormName)
    .then(surveyForm => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [surveyForm]));
...
router.post("/delete-multy", (req, res, next) => {
  ...
  surveyFormsModel
    .deleteSurveyForms(surveyFormIds, { task: "delete-many" })
    .then(surveyForms => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", surveyForms));
