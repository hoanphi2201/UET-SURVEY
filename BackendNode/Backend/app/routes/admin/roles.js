...
router.get("/all", (req, res, next) => {
  rolesModel
    ...
    .then(roles => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", roles));
    })
    .catch(error =>
      res
...
router.get("/", async (req, res, next) => {
  ...
  params.paging.total = await rolesModel
    .countRoles(params)
...
router.get("/:roleId", async (req, res, next) => {
  ...
  await rolesModel
    ...
    .then(role => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [role]));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.delete("/:roleId", (req, res, next) => {
  ...
  if (req.user.role.id === roleId) {
    return res
      .status(400)
      .json(
        new Response(true, 400, "error", "admin.layout.DELETE_ROLE_YOURSELF")
      );
  }
  rolesModel
    .deleteRoles(roleId, { task: "delete-one" })
    .then(role => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [role]));
...
router.put("/:roleId", (req, res, next) => {
  ...
  rolesModel
    .saveRole(role, roleId, { task: "update" })
    .then(role => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [role]));
...
router.post("/delete-multy", (req, res, next) => {
  ...
  rolesModel
    .deleteRoles(roleIds, { task: "delete-many" })
    .then(roles => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", roles));
...
router.post("/", (req, res, next) => {
  const role = req.body ? req.body : {};
  rolesModel
    .saveRole(role, null, { task: "create" })
    .then(role => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [role]));
