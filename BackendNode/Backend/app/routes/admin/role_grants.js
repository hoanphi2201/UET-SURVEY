...
router.get("/", async (req, res, next) => {
  ...
  params.paging.total = await roleGrantsModel
    .countRoleGrants(params)
    .catch(error => res.status(error.statusCode || 400).json(error));
  await roleGrantsModel
    .listRoleGrants(params)
    .then(roleGrants => {
      res
        .status(200)
        .json(
...
router.get("/:roleGrantId", (req, res, next) => {
  ...
  roleGrantsModel
    .getRoleGrantsById(roleGrantId)
    .then(roleGrant => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [roleGrant]));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.get("/roleGrantname/:roleGrantname", (req, res, next) => {
  ...
  roleGrantsModel
    .getRoleGrantsByRoleGrantname(roleGrantname)
    .then(roleGrant => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [roleGrant]));
...
router.delete("/:roleGrantId", (req, res, next) => {
  const roleGrantId = paramsHelper.getParam(req.params, "roleGrantId", "");
  roleGrantsModel
    .deleteRoleGrants(roleGrantId, { task: "delete-one" })
    .then(roleGrant => {
...
router.put("/:roleGrantId", (req, res, next) => {
  ...
  roleGrantsModel
    .saveRoleGrant(roleGrant, roleGrantId, { task: "update" })
    .then(roleGrant => {
      res.json(new Response(false, 200, "success", "Success", [roleGrant]));
    })
    .catch(error =>
...
router.post("/delete-multy", (req, res, next) => {
  ...
  roleGrantsModel
    .deleteRoleGrants(roleGrantIds, { task: "delete-many" })
    .then(roleGrants => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", roleGrants));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.post("/", (req, res, next) => {
  const roleGrant = req.body ? req.body : {};
  roleGrantsModel
    .saveRoleGrant(roleGrant, null, { task: "create" })
    .then(roleGrant => {
      res.json(new Response(false, 200, "success", "Success", [roleGrant]));
    })
    .catch(error =>
