...
router.get("/search", async (req, res, next) => {
  ...
  await surveyCollectorsModel
    .searchSurveyCollectorsByName(name, { limit })
    .then(surveyCollectors => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", surveyCollectors));
    })
    .catch(error =>
      res
...
router.get("/", async (req, res, next) => {
  ...
  params.paging.total = await surveyCollectorsModel
    .countSurveyCollectors(params)
    .catch(error => res.status(error.statusCode || 400).json(error));
  await surveyCollectorsModel
    .listSurveyCollectors(params)
    .then(surveyCollectors => {
      res
        .status(200)
        .json(
...
router.get("/:surveyCollectorId", async (req, res, next) => {
  ...
  await surveyCollectorsModel
    .getSurveyCollectorsById(surveyCollectorId)
...
router.get(
  ...
  async (req, res, next) => {
    const surveyCollectorName = paramsHelper.getParam(
      req.params,
      "surveyCollectorName",
      ""
    );
    await surveyCollectorsModel
      .getSurveyCollectorsBySurveyCollectorname(surveyCollectorName)
      .then(surveyCollector => {
        res
          .status(200)
          .json(
...
router.delete("/:surveyCollectorId", (req, res, next) => {
  ...
  surveyCollectorsModel
    .deleteSurveyCollectors(surveyCollectorId, { task: "delete-one" })
    .then(surveyCollector => {
      res.json(
        new Response(false, 200, "success", "Success", [surveyCollector])
      );
...
router.put("/change-role/:surveyCollectorId", (req, res, next) => {
  const params = {
    surveyCollectorId: paramsHelper.getParam(
      req.params,
      "surveyCollectorId",
      ""
    ),
...
router.put("/:surveyCollectorId", (req, res, next) => {
  ...
  surveyCollectorsModel
    .saveSurveyCollector(surveyCollector, surveyCollectorId, { task: "update" })
    .then(surveyCollector => {
      res.json(
        new Response(false, 200, "success", "Success", [surveyCollector])
      );
...
router.post("/delete-multy", (req, res, next) => {
  const surveyCollectorIds = paramsHelper.getParam(
    req.body,
    "surveyCollectorIds",
    ""
  );
  if (!Array.isArray(surveyCollectorIds)) {
    return res
      .status(400)
      .json(new Response(true, 400, "error", "admin.layout.IS_ARRAY"));
  }
  surveyCollectorsModel
    .deleteSurveyCollectors(surveyCollectorIds, { task: "delete-many" })
    .then(surveyCollectors => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", surveyCollectors));
...
router.post("/", (req, res, next) => {
  const surveyCollector = req.body ? req.body : {};
  surveyCollectorsModel
    .saveSurveyCollector(surveyCollector, null, { task: "create" })
    .then(surveyCollector => {
      res.json(
        new Response(false, 200, "success", "Success", [surveyCollector])
      );
