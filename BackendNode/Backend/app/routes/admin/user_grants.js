...
router.get("/", async (req, res, next) => {
  ...
  params.paging.total = await userGrantsModel
    .countUserGrants(params)
    .catch(error => res.status(error.statusCode || 400).json(error));
  await userGrantsModel
    .listUserGrants(params)
    .then(userGrants => {
      res
        .status(200)
        .json(
...
router.get("/:userGrantId", (req, res, next) => {
  ...
  userGrantsModel
    .getUserGrantsById(userGrantId)
    .then(userGrant => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [userGrant]));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.get("/userGrantname/:userGrantname", (req, res, next) => {
  ...
  userGrantsModel
    .getUserGrantsByUserGrantname(userGrantname)
    .then(userGrant => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [userGrant]));
...
router.delete("/:userGrantId", (req, res, next) => {
  const userGrantId = paramsHelper.getParam(req.params, "userGrantId", "");
  userGrantsModel
    .deleteUserGrants(userGrantId, { task: "delete-one" })
    .then(userGrant => {
...
router.put("/:userGrantId", (req, res, next) => {
  ...
  userGrantsModel
    .saveUserGrant(userGrant, userGrantId, { task: "update" })
    .then(userGrant => {
      res.json(new Response(false, 200, "success", "Success", [userGrant]));
    })
    .catch(error =>
...
router.post("/delete-multy", (req, res, next) => {
  ...
  userGrantsModel
    .deleteUserGrants(userGrantIds, { task: "delete-many" })
    .then(userGrants => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", userGrants));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.post("/", (req, res, next) => {
  const userGrant = req.body ? req.body : {};
  userGrantsModel
    .saveUserGrant(userGrant, null, { task: "create" })
    .then(userGrant => {
      res.json(new Response(false, 200, "success", "Success", [userGrant]));
    })
    .catch(error =>
