...
router.get("/all", (req, res, next) => {
  surveyFoldersModel
    .listAllSurveyFolders()
    .then(surveyFolders => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", surveyFolders));
    })
    .catch(error =>
      res
...
router.get("/", async (req, res, next) => {
  ...
  params.paging.total = await surveyFoldersModel
    .countSurveyFolders(params)
    .catch(error => res.status(error.statusCode || 400).json(error));
  await surveyFoldersModel
    .listSurveyFolders(params)
    .then(surveyFolders => {
      res
        .status(200)
        .json(
...
router.get("/:surveyFolderId", async (req, res, next) => {
  const surveyFolderId = paramsHelper.getParam(
    req.params,
    "surveyFolderId",
    ""
  );
  await surveyFoldersModel
    .getSurveyFoldersById(surveyFolderId)
    .then(surveyFolder => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [surveyFolder]));
    })
    .catch(error =>
      res
        .status(error.statusCode || 400)
...
router.put("/:surveyFolderId", (req, res, next) => {
  ...
  surveyFoldersModel
    .saveSurveyFolder(surveyFolder, surveyFolderId, { task: "update" })
    .then(surveyFolder => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [surveyFolder]));
...
router.post("/delete-multy", (req, res, next) => {
  const surveyFolderIds = paramsHelper.getParam(
    req.body,
    "surveyFolderIds",
    ""
  );
  ...
  surveyFoldersModel
    .deleteSurveyFolders(surveyFolderIds, { task: "delete-many" })
    .then(surveyFolders => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", surveyFolders));
...
router.post("/", (req, res, next) => {
  const surveyFolder = req.body ? req.body : {};
  surveyFoldersModel
    .saveSurveyFolder(surveyFolder, null, { task: "create" })
    .then(surveyFolder => {
      res
        .status(200)
        .json(new Response(false, 200, "success", "Success", [surveyFolder]));
