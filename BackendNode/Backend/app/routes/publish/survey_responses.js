...
router.post("/", async (req, res, next) => {
  ...
  if (!surveyResponse) {
    return res.status(400).json(new Response(true, 400, "error", "default.layout.NOT_HAVE_RESPONSE"));
  }
  const surveyCollector = await surveyCollectorsModel.getSurveyCollectorsById(surveyResponse.surveyCollectorId).catch(error => {
    return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
  });
  if (!surveyCollector) {
    return res.status(400).json(new Response(true, 400, "error", "default.layout.COLLECTOR_NOT_EXIST"));
  }
  if (surveyCollector.status === "CLOSED") {
    return res.status(400).json(new Response(true, 400, "error", "default.layout.HAVE_BEEN_CLOSE"));
  }
  if (surveyCollector.passwordEnabled) {
    if (surveyResponse.collectorPassword !== surveyCollector.password) {
      return res.status(400).json(new Response(true, 400, "error", "default.layout.INCORRECT_PASSWORD"));
    }
  }
  if (!surveyCollector.allowMultipleResponses) {
    const ipResponse = await surveyResponsesModel.getSurveyResponsesByIpAddress({
    ...
    if (ipResponse) {
      return res.status(400).json(new Response(true, 400, "error", "default.layout.HAVE_ALREADY_SUBMIT_RESPONSE"));
    }
  }
  const saceSurveyResponse = await surveyResponsesModel.saveSurveyResponse(surveyResponse, null, { task: "create" }).catch(error => {
    return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
  });
  ...
  if (actionClose) {
    ...
    await surveyCollectorsModel.saveSurveyCollector(collectorUpDate, surveyResponse.surveyCollectorId, {
      task: "update"
    }).catch(error => {
      return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
    });
  }
  return res.status(200).json(new Response(false, 200, "success", "Success", [saceSurveyResponse]));
});
