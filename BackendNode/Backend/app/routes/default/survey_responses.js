...
router.get("/count-all-responses-typical-time-spent", async (req, res, next) => {
  const totalResponse = await surveyResponsesModel.countAllSurveyResponses().catch(error => {
    return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
  });
  const totalResponseComplete = await surveyResponsesModel.countAllSurveyResponses({ json: { $ne: null } }).catch(error => {
    return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
  });
  const countTypicalTimeSpent = await surveyResponsesModel.countTypicalTimeSpent().catch(error => {
    return res.status(error.statusCode || 400).json(new Response(true, 400, "error", error.message));
  });
